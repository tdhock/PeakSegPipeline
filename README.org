...
- *Single-sample peak calling using PeakSegFPOP.* This repository
  includes a limited memory implementation of PeakSegFPOP, a Generalized
  Functional Pruning Optimal Partitioning algorithm
  (GFPOP, [[https://arxiv.org/abs/1703.03352][arXiv:1703.03352]]), which is used to predict preliminary peaks for
...
Our paper (TODO:LINK) describes two new algorithms, which are
...
*** PeakSegPipeline::problem.PeakSegFPOP 

The first algorithm is Generalized Functional Pruning Optimal
Partitioning (GFPOP) with up-down constraints between adjacent segment
means. To use this algorithm you must give the folder name (not the
coverage.bedGraph file name) to the problem.PeakSegFPOP function:

#+BEGIN_SRC R
PeakSegPipeline::problem.PeakSegFPOP("sampleID/problems/c:0-6", "0.1")
#+END_SRC
...
*** PeakSegPipeline::problem.sequentialSearch
...
#+BEGIN_SRC R
PeakSegPipeline::problem.sequentialSearch("sampleID/problems/c:0-6", 17L)
#+END_SRC
